...
// configuration for Split-K GEMV
template <int _Mb, int _Kb, int _PIPE_DEPTH, int _SPLIT_K = 1>
struct gemv_splitk_config {
    static constexpr int Mb = _Mb;
    static constexpr int Nb = 16;
    static constexpr int Kb = _Kb;
    static constexpr int PIPE_DEPTH = _PIPE_DEPTH;
...
// global memory descriptors for Split-K GEMV
template <typename C>
struct gemv_splitk_globals {
    using a_tile = st_bf<C::Mb, C::Kb>;
    using x_tile = st_bf<C::Nb, C::Kb>;
    using y_tile = st_bf<C::Mb, C::Nb>;
    ...
    x_gl x;
    y_gl y;
    float* y_partial;  // partial results for reduction (fp32)

    int M, K;

    // Grid: (M_blocks, SPLIT_K)
    __host__ __inline__ dim3 grid() { return dim3(M / C::Mb, C::SPLIT_K); }
    __host__ __inline__ dim3 block() { return dim3(C::NUM_THREADS); }
    __host__ __inline__ int dynamic_shared_memory() {
        constexpr size_t mem = sizeof(a_tile) * C::PIPE_DEPTH +
                               sizeof(x_tile) * C::PIPE_DEPTH +
...
// Split-K GEMV kernel: each block handles a subset of K iterations
// Multiple blocks accumulate partial results and atomically add to output
template <typename C>
__launch_bounds__(C::NUM_THREADS, 1)
__global__ void gemv_splitk_kernel(const __grid_constant__ gemv_splitk_globals<C> g) {
    using G = gemv_splitk_globals<C>;
    ...
    const int total_k_iters = g.K / C::Kb;
    const int k_iters_per_slice = total_k_iters / C::SPLIT_K;
    const int k_start = k_slice * k_iters_per_slice;
    const int k_end = (k_slice == C::SPLIT_K - 1) ? total_k_iters : k_start + k_iters_per_slice;
    ...
    if (warpgroupid == C::NUM_CONSUMERS) {
        ...
        if (warpgroup::warpid() == 3 && warp::laneid() == 0) {
            // TMA loader thread
            int input_ring = 0;
            const int num_iters = k_end - k_start;
            ...
            for (int idx = 0; idx < num_iters; idx++) {
                ...
                tma::expect(inputs_arrived[input_ring], a_smem[0], x_smem[0]);
                tma::load_async(a_smem[input_ring], g.a, {row_block, k_start + idx}, inputs_arrived[input_ring]);
                tma::load_async(x_smem[input_ring], g.x, {0, k_start + idx}, inputs_arrived[input_ring]);
            ...
            // remaining use mma
            for (int idx = 1; idx < num_iters; idx++) {
                wait(inputs_arrived[input_ring], get_phasebit<0>(bitfield, input_ring));
                update_phasebit<0>(bitfield, input_ring);
                mma_ABt(d_tt, a_smem[input_ring], x_smem[input_ring], inputs_finished[input_ring]);
                input_ring = ring_advance<C::PIPE_DEPTH>(input_ring);
        ...
        // Atomic reduction to partial results buffer
        // Each thread extracts its portion and atomically adds
        const int tid = threadIdx.x;
        const int warp_id = tid / WARP_THREADS;
        const int lane_id = tid % WARP_THREADS;

        // y_smem is Mb x Nb, we need column 0
        // Each warp handles some rows
        const int rows_per_warp = C::Mb / (C::NUM_CONSUMERS * WARPGROUP_WARPS);
        const int base_row = row_block * C::Mb + warp_id * rows_per_warp;

        if (lane_id < rows_per_warp && warp_id < C::NUM_CONSUMERS * WARPGROUP_WARPS) {
            const int row = base_row + lane_id;
            // Read from y_smem (Mb x Nb layout), column 0
            float partial = __bfloat162float(y_smem.data[lane_id + warp_id * rows_per_warp * C::Nb]);
            atomicAdd(&g.y_partial[row], partial);
        }
    }
...
// Kernel to convert partial fp32 results to final bf16 output
__global__ void convert_partial_to_bf16(bf16* y, const float* y_partial, int M) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx < M) {
        y[idx] = __float2bfloat16(y_partial[idx]);
    }
}
...
// Split-K benchmark function
template <typename C>
__host__ double run_gemv_splitk_benchmark(size_t M, size_t K, bool ncu = false) {
    std::cout << "--------------------  M=" << M << " K=" << K << " SPLIT_K=" << C::SPLIT_K << "  --------------------\n";
    std::cout << "Template: Mb=" << C::Mb << " Nb=" << C::Nb << " Kb=" << C::Kb
              << " PIPE_DEPTH=" << C::PIPE_DEPTH << " SPLIT_K=" << C::SPLIT_K << "\n";
    ...
    int l2_cache_size;
    cudaDeviceGetAttribute(&l2_cache_size, cudaDevAttrL2CacheSize, 0);
    const size_t arg_size = 2 * (size_t(M) * K + K + M) + 4 * M;  // bf16 bytes + fp32 partial
    const size_t ideal_arg_size = size_t(l2_cache_size) * 3;
    const int arg_group_count = (arg_size > ideal_arg_size) ? 1 : int(ideal_arg_size / arg_size) + 1;
    ...
    std::vector<bf16*> d_x_tiles(arg_group_count);
    std::vector<bf16*> d_y_tiles(arg_group_count);
    std::vector<bf16*> d_y(arg_group_count);
    std::vector<float*> d_y_partial(arg_group_count);  // fp32 partial results for atomic reduction
    bf16* d_y_ref;
    ...
    // Prepare kernel inputs
    std::vector<gemv_splitk_globals<C>> g;
    for (int i = 0; i < arg_group_count; i++) {
        typename gemv_splitk_globals<C>::a_gl Ag{d_A[i], nullptr, nullptr, M, K};
        typename gemv_splitk_globals<C>::x_gl Xg{d_x_tiles[i], nullptr, nullptr, (size_t)C::Nb, (size_t)K};
        typename gemv_splitk_globals<C>::y_gl Yg{d_y_tiles[i], nullptr, nullptr, M, C::Nb};
        g.push_back(gemv_splitk_globals<C>{Ag, Xg, Yg, d_y_partial[i], (int)M, (int)K});
    }

    // Set kernel attributes
    CUDACHECK(cudaFuncSetAttribute(gemv_splitk_kernel<C>, cudaFuncAttributeMaxDynamicSharedMemorySize,
                                   g[0].dynamic_shared_memory()));
    ...
    for (int i = 0; i < num_warmups; i++) {
        int idx = i % arg_group_count;
        // Zero partial results before each kernel
        CUDACHECK(cudaMemset(d_y_partial[idx], 0, M * sizeof(float)));
        gemv_splitk_kernel<C><<<g[idx].grid(), g[idx].block(), g[idx].dynamic_shared_memory()>>>(g[idx]);
    }
    CUDACHECK(cudaDeviceSynchronize());
    ...
    for (int i = 0; i < num_iters; i++) {
        ...
        CUDACHECK(cudaMemset(d_y_partial[idx], 0, M * sizeof(float)));
        gemv_splitk_kernel<C><<<g[idx].grid(), g[idx].block(), g[idx].dynamic_shared_memory()>>>(g[idx]);
        // Convert partial fp32 results to bf16
        convert_partial_to_bf16<<<(M + 255) / 256, 256>>>(d_y[idx], d_y_partial[idx], M);
    }
    CUDACHECK(cudaEventRecord(stop));
    CUDACHECK(cudaEventSynchronize(stop));
...
__host__ int main() {
    ...
    std::cout << "\n========== Split-K GEMV (SPLIT_K=2) ==========\n";
    N = 4096;
    run_gemv_splitk_benchmark<gemv_splitk_config<128, 128, 4, 2>>(N, N, ncu);
    ...
    N = 32768;
    run_gemv_splitk_benchmark<gemv_splitk_config<128, 64, 4, 2>>(N, N, ncu);

    std::cout << "\n========== Split-K GEMV (SPLIT_K=4) ==========\n";
    N = 4096;
    run_gemv_splitk_benchmark<gemv_splitk_config<128, 128, 4, 4>>(N, N, ncu);
    ...
    N = 32768;
    run_gemv_splitk_benchmark<gemv_splitk_config<128, 64, 4, 4>>(N, N, ncu);

    std::cout << "\n========== Split-K GEMV (SPLIT_K=8) ==========\n";
    N = 4096;
    run_gemv_splitk_benchmark<gemv_splitk_config<128, 128, 4, 8>>(N, N, ncu);
    ...
    N = 32768;
    run_gemv_splitk_benchmark<gemv_splitk_config<128, 64, 4, 8>>(N, N, ncu);
